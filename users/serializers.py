...
from .models import User, EmailVerification, WebAuthnCredential
...
class WebAuthnCredentialSerializer(serializers.ModelSerializer):
    """
    Сериализатор для отображения passkey пользователя.

    Намеренно НЕ возвращает credential_id и public_key:
    они содержат чувствительные криптографические данные.
    Клиенту нужны только метаданные для отображения в интерфейсе.
    """
